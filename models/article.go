...
type Article struct {
	...
	CreatedAt    time.Time `orm:"type(datetime);column(created_at);" description:"添加时间" json:"created_at"`
	UpdatedAt    time.Time `orm:"type(datetime);column(updated_at);" description:"最后更新时间" json:"updated_at"`
	PublishedAt  time.Time `orm:"type(datetime);column(published_at);" description:"发布时间" json:"published_at"`
}
